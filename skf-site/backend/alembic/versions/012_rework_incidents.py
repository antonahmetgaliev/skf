...
def upgrade() -> None:
    ...
    if "incident_windows" in existing:
        with op.batch_alter_table("incident_windows") as batch_op:
            batch_op.alter_column("championship_id", existing_type=sa.Integer(), nullable=True)
            batch_op.alter_column("championship_name", existing_type=sa.String(200), nullable=True)
            batch_op.alter_column("race_id", existing_type=sa.Integer(), nullable=True)
            batch_op.add_column(sa.Column("date", sa.String(20), nullable=True))

    # Recreate incidents with new schema
    ...
    # New incident_drivers join table
    if "incident_drivers" not in inspector.get_table_names():
        op.create_table(
            "incident_drivers",
            sa.Column("id", UUID(as_uuid=True), primary_key=True),
            sa.Column(
    ...
    # Recreate incident_resolutions with per-driver FK
    if "incident_resolutions" not in inspector.get_table_names():
        op.create_table(
